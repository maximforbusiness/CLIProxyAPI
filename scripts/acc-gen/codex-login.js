        ...
        try {
            ...
            for (const phoneCandidate of phoneCandidates) {
                ...
                if (deliveryMode === 'whatsapp') {
                    throw new Error('phone_requires_whatsapp');
                }
                if (deliveryMode === 'rejected') {
                    throw new Error('phone_rejected_by_openai');
                }

                codeInputResult = await waitForAnySelector(page, codeInputSelectors, 3500);
                if (codeInputResult) break;

                // If we don't have code input quickly, do not burn time.
                if (deliveryMode === 'unknown' || deliveryMode === 'sms' || deliveryMode === 'rejected') {
                    continue;
                }
            }

            if (!codeInputResult || !codeInputResult.handle) {
                throw new Error(deliveryMode === 'whatsapp' ? 'phone_requires_whatsapp' : 'sms_code_input_not_found');
            }
            ...
            const canTryNextCountry = /hero_sms_code_timeout|STATUS_WAIT_CODE|sms_code_input_not_found|hero_sms_get_number_failed|phone_input_not_found|phone_verification_still_required|phone_requires_whatsapp/i.test(lastReason);
...
async function getVerificationCodeFromIMAP(email, password, waitSeconds = 60, waitForNewEmail = false) {
    ...
    const IMAP_PORT = 993;
    const appPassword = String(process.env.CODEX_GMAIL_APP_PASSWORD || password || '').replace(/\s+/g, '');

    console.log(`Gmail IMAP: ${IMAP_HOST}:${IMAP_PORT}`);
...
async function performLogin(authUrl) {
    ...
    try {
        ...
        if (isAboutYouPage) {
            ...
            if (visibleTextInputs.length <= 3 && hasNameField && hasAgeField) {
                console.log('About-you strict mode: filling NAME then AGE');
                ...
                await sleep(600);
                await takeScreenshot(page, '08-about-you-strict-name-age');
            }
        ...
        if (newPageUrl.includes('about-you')) {
            ...
            // Retry once if still on about-you after phone flow.
            if (page.url().includes('about-you')) {
