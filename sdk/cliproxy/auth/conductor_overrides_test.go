...
func TestManagerExecute_DisablesBlockedAuthAndFallsBack(t *testing.T) {
	m := NewManager(nil, nil, nil)
	executor := &authFallbackExecutor{
		id: "openai",
		executeErrors: map[string]error{
			"aa-blocked-auth": &Error{
				...
				Message:    `{"error":{"code":"account_blocked","message":"Account is blocked due to policy violation"}}`,
			},
		},
	}
	m.RegisterExecutor(executor)
	...
	model := "gpt-5"
	badAuth := &Auth{ID: "aa-blocked-auth", Provider: "openai"}
	...
	request := cliproxyexecutor.Request{Model: model}
	for i := 0; i < 2; i++ {
		resp, errExecute := m.Execute(context.Background(), []string{"openai"}, request, cliproxyexecutor.Options{})
		if errExecute != nil {
			t.Fatalf("execute %d error = %v, want success", i, errExecute)
		}
		if string(resp.Payload) != goodAuth.ID {
	...
	if !updatedBad.Disabled {
		t.Fatalf("expected blocked auth to be disabled")
	}
	if updatedBad.Status != StatusDisabled {
		t.Fatalf("status = %q, want %q", updatedBad.Status, StatusDisabled)
	}
}

func TestManager_MarkResult_ParsesRetryAfterFromProviderErrorBody(t *testing.T) {
	m := NewManager(nil, nil, nil)
	...
	m.MarkResult(context.Background(), Result{
		...
		Error: &Error{
			...
			Message:    `{"error":{"message":"rate limit","retry_after":17}}`,
		},
	})
	...
	if !state.Quota.Exceeded {
		t.Fatalf("expected quota exceeded flag to be set")
	}
	if state.NextRetryAfter.IsZero() {
		t.Fatalf("expected NextRetryAfter to be set")
	}
	wait := state.NextRetryAfter.Sub(started)
	if wait < 15*time.Second || wait > 25*time.Second {
		t.Fatalf("cooldown = %v, want approximately 17s", wait)
	}
}

func TestIsPermanentAccountBlockResultError_DetectsIFlowBlockedPhrase(t *testing.T) {
	err := &Error{
		HTTPStatus: 434,
		Message:    `{"status":"434","msg":"Access to the current AK has been blocked due to unauthorized requests","body":null}`,
	}
	if !isPermanentAccountBlockResultError(err) {
		t.Fatalf("expected blocked phrase to be treated as permanent account block")
	}
}
