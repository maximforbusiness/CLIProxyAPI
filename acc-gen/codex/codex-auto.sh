...
RUN_LOCK_FD=""
CODEX_CALLBACK_PORT="${CODEX_CALLBACK_PORT:-1456}"
...
extract_callback_url() {
    ...
    if [ -z "$callback_url" ]; then
        callback_url="$(printf '%s\n' "$input" | grep -oP 'http://localhost:'"$CODEX_CALLBACK_PORT"'/auth/callback[^\s"]*' | tail -n 1)"
    fi
...
callback_was_delivered_locally() {
    local login_log="$1"
    [ -n "$login_log" ] && [ -f "$login_log" ] && grep -q 'Local redirect without code: http://localhost:'"$CODEX_CALLBACK_PORT"'/success' "$login_log" 2>/dev/null
}
...
kill_port_1455_listener() {
    ...
    echo "[callback-debug] cleaning port "$CODEX_CALLBACK_PORT" listener PID(s): $(echo "$pids" | tr '\n' ' ')"
    while IFS= read -r pid; do
        [ -n "$pid" ] || continue
        kill_pid_soft_then_hard "$pid"
    done <<EOF
...
start_cli_auth_with_recover() {
    ...
    for startup_attempt in 1 2 3; do
        if ss -ltn 2>/dev/null | grep -q ":$CODEX_CALLBACK_PORT "; then
            echo "[callback-debug] port "$CODEX_CALLBACK_PORT" busy before startup attempt ${startup_attempt}; cleaning listener"
            kill_port_1455_listener
            sleep 1
        fi
...
wait_for_port_1455() {
    for i in {1..15}; do
        if netstat -tuln 2>/dev/null | grep -q ":$CODEX_CALLBACK_PORT " || ss -tuln 2>/dev/null | grep -q ":$CODEX_CALLBACK_PORT "; then
            return 0
        fi
        sleep 1
    done
...
for ((idx=START_INDEX; idx<END_INDEX; idx++)); do
    ...
    for ((attempt=0; attempt<ATTEMPT_LIMIT; attempt++)); do
        ...
        if ! start_cli_auth_with_recover "$CURRENT_AUTH_LOG"; then
            echo "✗ Failed to start cli-proxy-api auth listener on port "$CODEX_CALLBACK_PORT" after retries"
        ...
        # Wait until port "$CODEX_CALLBACK_PORT" is actually listening
        echo "Waiting for port "$CODEX_CALLBACK_PORT" to be ready..."
        PORT_READY=false
        for port_wait in $(seq 1 15); do
            if ss -tuln 2>/dev/null | grep -q ":$CODEX_CALLBACK_PORT " || netstat -tuln 2>/dev/null | grep -q ":$CODEX_CALLBACK_PORT "; then
                PORT_READY=true
                break
            fi
            sleep 1
        ...
        if [ "$PORT_READY" != true ]; then
            echo "Warning: port "$CODEX_CALLBACK_PORT" not ready after 15s"
        fi
        ...
        echo "Waiting for port "$CODEX_CALLBACK_PORT"..."
        if ! wait_for_port_1455; then
            echo "✗ Port "$CODEX_CALLBACK_PORT" is not ready"
        ...
        if [ -n "${CODEX_SCREENSHOTS:-}" ]; then
            NODE_ENV_ARGS+=("CODEX_SCREENSHOTS=$CODEX_SCREENSHOTS")
        fi
        if [ -n "$SMS_SERVICES" ]; then
        ...
        env "${NODE_ENV_ARGS[@]}" xvfb-run -a -s "-screen 0 1920x1080x24" node "$SCRIPT_DIR/codex-login.js" "$LOGIN_URL" "$idx" 2>&1 | tee "$LOGIN_LOG"
        ...
        if [ -z "$CALLBACK_URL" ] \
            ...
            if [ "$RESULT_REASON" = "password_verify_401" ] \
                ...
                || [[ "$RESULT_REASON" == password_input_missing_after_email* ]] \
                ...
                env "${NODE_ENV_ARGS_SIGNUP[@]}" xvfb-run -a -s "-screen 0 1920x1080x24" node "$SCRIPT_DIR/codex-login.js" "$LOGIN_URL" "$idx" 2>&1 | tee -a "$LOGIN_LOG"
                LOGIN_EXIT=${PIPESTATUS[0]}
        ...
        if [ -n "$CALLBACK_URL" ]; then
            ...
            if callback_was_delivered_locally "$LOGIN_LOG"; then
                ...
                echo "Prompt handoff not available, sending callback via HTTP to cliproxyapi on port "$CODEX_CALLBACK_PORT"..."
                echo "[callback-debug] port "$CODEX_CALLBACK_PORT" snapshot before HTTP submit:"
                ss -tuln 2>/dev/null | grep ":$CODEX_CALLBACK_PORT " || netstat -tuln 2>/dev/null | grep ":$CODEX_CALLBACK_PORT " || echo "[callback-debug] no listener reported by ss/netstat"
