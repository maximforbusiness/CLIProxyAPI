...
// wrapQwenError wraps an HTTP error response for Qwen-specific quota handling.
// A 403 insufficient_quota is treated as a hard daily quota exhaustion and
// gets a retry hint until the next Beijing midnight. A 429 is left without a
// long retry hint so the auth layer can keep rotating instead of parking
// potentially healthy keys for the rest of the day.
func wrapQwenError(ctx context.Context, httpCode int, body []byte) (errCode int, retryAfter *time.Duration) {
	...
	// Qwen commonly reports hard quota exhaustion as 403 insufficient_quota, while
	// 429 responses can be transient and should not be stretched into an all-day
	// cooldown by the executor.
	if (httpCode == http.StatusForbidden || httpCode == http.StatusTooManyRequests) && isQwenQuotaError(body) {
		...
		if httpCode == http.StatusTooManyRequests {
			helps.LogWithRequestID(ctx).Warnf("qwen quota-like 429 received, leaving retryAfter unset for auth rotation")
			return errCode, nil
		}
		cooldown := timeUntilNextDay()
		retryAfter = &cooldown
		helps.LogWithRequestID(ctx).Warnf("qwen hard quota exceeded (http %d -> %d), cooling down until tomorrow (%v)", httpCode, errCode, cooldown)
	}
