...
func TestEnsureQwenSystemMessage_MergeStringSystem(t *testing.T) {
	...
	part0Text := parts[0].Get("text").String()
	if (part0Text != "" && part0Text != "You are Qwen Code.") || parts[0].Get("cache_control.type").String() != "ephemeral" {
...
func TestWrapQwenError_403QuotaSetsLongRetryAfter(t *testing.T) {
	body := []byte(`{"error":{"code":"insufficient_quota","message":"quota exceeded","type":"insufficient_quota"}}`)

	status, retryAfter := wrapQwenError(context.Background(), http.StatusForbidden, body)
	...
	if *retryAfter > 24*time.Hour {
		t.Fatalf("retryAfter = %v, want <= 24h", *retryAfter)
	}
}

func TestWrapQwenError_429QuotaLeavesRetryAfterUnset(t *testing.T) {
	body := []byte(`{"error":{"code":"insufficient_quota","message":"quota exceeded","type":"insufficient_quota"}}`)

	status, retryAfter := wrapQwenError(context.Background(), http.StatusTooManyRequests, body)
	if status != http.StatusTooManyRequests {
		t.Fatalf("status = %d, want %d", status, http.StatusTooManyRequests)
	}
	if retryAfter != nil {
