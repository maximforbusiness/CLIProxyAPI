...
func (e *IFlowExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if embeddedErr := detectIFlowEmbeddedError(httpResp.Header, data); embeddedErr != nil {
		helps.LogWithRequestID(ctx).Debugf("iflow embedded error detected in successful HTTP response: %s", helps.SummarizeErrorBody(httpResp.Header.Get("Content-Type"), data))
		err = *embeddedErr
		return resp, err
	}
...
func (e *IFlowExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if !strings.Contains(contentType, "text/event-stream") {
		...
		helps.AppendAPIResponseChunk(ctx, e.cfg, data)
		if embeddedErr := detectIFlowEmbeddedError(httpResp.Header, data); embeddedErr != nil {
			helps.LogWithRequestID(ctx).Debugf("iflow embedded stream error detected in successful HTTP response: %s", helps.SummarizeErrorBody(httpResp.Header.Get("Content-Type"), data))
			err = *embeddedErr
			return nil, err
		}
		err = statusErr{code: http.StatusBadGateway, msg: "iflow executor: expected event-stream response for streaming request"}
		return nil, err
	}
...
func detectIFlowEmbeddedError(headers http.Header, body []byte) *statusErr {
	...
	var parts []string
	if msg := strings.TrimSpace(gjson.GetBytes(trimmed, "msg").String()); msg != "" {
		parts = append(parts, msg)
	}
	if msg := strings.TrimSpace(gjson.GetBytes(trimmed, "message").String()); msg != "" {
		parts = append(parts, msg)
	}
	if msg := strings.TrimSpace(gjson.GetBytes(trimmed, "error.message").String()); msg != "" {
		parts = append(parts, msg)
	}
	if code := strings.TrimSpace(gjson.GetBytes(trimmed, "error.code").String()); code != "" {
		parts = append(parts, code)
	}
	normalized := normalizeIFlowErrorText(strings.Join(parts, " "))

	statusCode, hasStatus := parseIFlowStatusCode(trimmed)
	hasErrorObject := gjson.GetBytes(trimmed, "error").Exists()
	errorMarkers := []string{
		"blocked",
		"suspended",
		"banned",
	...
	if !hasErrorObject && !hasStatus && !hasAnyIFlowToken(normalized, errorMarkers) {
		return nil
	}
	if hasStatus && statusCode >= http.StatusOK && statusCode < http.StatusMultipleChoices && !hasErrorObject && !hasAnyIFlowToken(normalized, errorMarkers) {
		return nil
	}

	mappedStatus := mapIFlowEmbeddedErrorStatus(normalized, statusCode)
	if mappedStatus < 400 || mappedStatus > 599 {
		mappedStatus = http.StatusBadGateway
	}
	out := &statusErr{code: mappedStatus, msg: string(trimmed)}
	if retryAfter := parseIFlowRetryAfter(headers, trimmed); retryAfter != nil {
		out.retryAfter = retryAfter
	}
	return out
}

func parseIFlowStatusCode(body []byte) (int, bool) {
	paths := []string{
		"status",
		"status_code",
		"error.status",
...
func mapIFlowEmbeddedErrorStatus(normalized string, fallback int) int {
	blockedTokens := []string{"blocked", "suspended", "banned", "revoked", "disabled"}
	if hasAnyIFlowToken(normalized, blockedTokens) {
		return http.StatusForbidden
	}
	unauthorizedTokens := []string{"unauthorized", "invalid api key", "authentication", "forbidden", "access denied"}
	if hasAnyIFlowToken(normalized, unauthorizedTokens) {
		return http.StatusUnauthorized
	}
	cooldownTokens := []string{"cooldown", "retry after", "try again in", "wait", "reset"}
	quotaTokens := []string{"quota", "rate limit", "too many requests", "resource exhausted"}
	if hasAnyIFlowToken(normalized, cooldownTokens) || hasAnyIFlowToken(normalized, quotaTokens) {
...
func parseIFlowRetryAfter(headers http.Header, body []byte) *time.Duration {
	if retryAfter := parseRetryAfterHeaderValue(headers.Get("Retry-After")); retryAfter != nil {
		return retryAfter
	}
	...
	for _, path := range secPaths {
		if value := parseIFlowDurationValue(gjson.GetBytes(body, path), time.Second); value != nil {
			return value
		}
	}
	...
	for _, path := range msPaths {
		if value := parseIFlowDurationValue(gjson.GetBytes(body, path), time.Millisecond); value != nil {
			return value
		}
	}

	resetTimePaths := []string{
		"error.reset_time",
		"reset_time",
		"error.resetTime",
		"resetTime",
	}
	for _, path := range resetTimePaths {
		if value := parseIFlowDurationValue(gjson.GetBytes(body, path), time.Second); value != nil {
			return value
		}
	}
...
func parseRetryAfterHeaderValue(raw string) *time.Duration {
...
func parseIFlowDurationValue(value gjson.Result, defaultUnit time.Duration) *time.Duration {
	if !value.Exists() {
		return nil
	}
	raw := strings.TrimSpace(value.String())
	...
	duration := time.Duration(number * float64(defaultUnit))
	if duration <= 0 {
		return nil
	}
	return &duration
...
func normalizeIFlowErrorText(text string) string {
	if text == "" {
		return ""
	}
...
func hasAnyIFlowToken(text string, tokens []string) bool {
	if text == "" {
		return false
	}
